...
void play_server(tcp::iostream &conn)
{
  ...
  for (long _i = 0; _i < bench_gt::trial; _i++) {
    ...
    std::cout << "server_input " << server_input << " bit " << binder_bit << "\n";
    START_TIMER(1);
...
void play_client(tcp::iostream &conn)
{
  ...
  for (long _i = 0; _i < bench_gt::trial; _i++)  {
    ...
    std::cout << "client_input: " << client_input << " bit " << s << "\n";
    gt_result.resize(1);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    long server_input = std::rand() % maximum;
    long blind_bit = std::rand() % 2;
    auto gt_res = GT(pub, enc_bits, server_input, blind_bit, bench_gt::bitlen);

    int s = decrypt_gt(prv, gt_res);
    assert((server_input > client_input) == (s ^ blind_bit));
  }
  return 0;
}
#endif
